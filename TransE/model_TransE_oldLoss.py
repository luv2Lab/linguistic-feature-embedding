#!/usr/bin/python3
# -*- coding:utf-8 -*-
# Author:KunXun Qi & YuanChen
# This is the code of the TransE model, and its loss function is the initial loss function of TransE, and the input is the pretrained vector.
...
class TransE:
    ...
    def transe_main(self):
        ...
        p_h = tf.nn.embedding_lookup(ent_embeddings, pos_h)
        p_t = tf.nn.embedding_lookup(ent_embeddings, pos_t)
        ...
        p_score = tf.reduce_sum(tf.reduce_mean(_p_score, reduction_indices=1, keep_dims=False), reduction_indices=1,
                                keep_dims=True)
        n_score = tf.reduce_sum(tf.reduce_mean(_n_score, reduction_indices=1, keep_dims=False), reduction_indices=1,
                                keep_dims=True)
